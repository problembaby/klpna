...
// 페이지 내 모든 .cls-bot-progress 요소에 대해
// 각 progress bar의 width와 숫자 퍼센트를 부드럽게 애니메이션 처리
*/

document.addEventListener("DOMContentLoaded", function () {
    const progresses = document.querySelectorAll('.cls-bot-progress');

    progresses.forEach(progress => {
        const progressFill = progress.querySelector('.progress-fill');
        const rateNum = progress.querySelector('.rate-num');
        const target = parseInt(progressFill.dataset.progress, 10);
        ...
        const counter = setInterval(() => {
            ...
            rateNum.textContent = Math.round(current) + '%';
        }, frameRate);
    });
});
